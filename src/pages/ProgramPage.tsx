...
function NewProgramForm({ onDone }: NewProgramFormProps) {
...
function ProgramDetail({ program, onClose }: ProgramDetailProps) {
...
export function ProgramPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {adding && <NewProgramForm onDone={() => setAdding(false)} />}
      {selectedProgram && (
        <ProgramDetail program={selectedProgram} onClose={() => setSelectedId(null)} />
      )}
      ...
      {isLoading ? (
        ...
        <p className="text-gray-500 text-sm">
          No programs yet. Create one to define your exercise order for each session.
        </p>
      ) : (
        <div className="space-y-2">
          {programs.map((program) => (
            <button
              key={program.id}
              onClick={() => setSelectedId(program.id === selectedId ? null : program.id)}
